...
{
    // Get the limit of the current data type
    T limit = std::numeric_limits<T>::max();
    T result = start;
    
    std::cout << "This is the current limit: " << limit << std::endl;
    for (unsigned long int i = 0; i <= steps; ++i)
    {   
        // Find the maximum size for the current data type being operated on
        if (result + increment >= limit) {

            // if overlfow/underflow set result to lowest/highest value
            result = limit;
        }
        else {
            // If there is no overflow, return the correctly computed value
            result += increment;
        }
    }
...
T subtract_numbers(T const& start, T const& decrement, unsigned long int const& steps)
{
    ...
    for (unsigned long int i = 0; i <= steps; ++i)
    {
        // Find the minimum size for the current data type being operated on
        if (result - decrement <= limit) {

            result = limit; // return the minumum value for the data type
        }
        else { 
            // If there is no underflow, return the correctly computed value
            result -= decrement;
        }
    }
...
void test_overflow()
{
    ...
    // If the operation was successful output result
    std::cout << "The operation was completed successfully. The result is: " << + result << std::endl;
    std::cout << std::endl;
    std::cout << "\Adding Numbers With Overflow (" << +start << ", " << +increment << ", " << (steps + 1) << ") = ";
    result = add_numbers<T>(start, increment, steps + 1);

    // set max limit for data type
    T limit = std::numeric_limits<T>::max();

    // If the operattion went outside the limit for the data type print the maximum value for that data type
        std::cout << "This operation resulted in an overflow." << std::endl;
        std::cout << "The max value of this data type is : " << +limit << std::endl;
        std::cout << std::endl;
...
void test_underflow()
{
    ...
    // whats our starting point
    const T start = std::numeric_limits<T>::min();

    std::cout << "Underflow Test of Type = " << typeid(T).name() << std::endl;
    // END DO NOT CHANGE

    //If the operation was successful without any overflow print success statement and final value
    std::cout << "Subtracting Numbers Without Overflow (" << +start << ", " << +decrement << ", " << steps << ") = ";
    auto result = subtract_numbers<T>(start, decrement, steps);
    std::cout << "This operation was completed successfully: The result is: " << + result << std::endl;

    std::cout << std::endl;

    // Set the minimum limit for each data type 
    T limit = std::numeric_limits<T>::min();
    // If an underflow occurs, print statement with minimum value allowed for the data type
    std::cout << "Subtracting Numbers With Overflow (" << +start << ", " << +decrement << ", " << (steps + 1) << ") = ";
    result = subtract_numbers<T>(start, decrement, steps + 1);

        std::cout << "The operation resulted in an underflow" << std::endl;
        std:: cout << "The minimum value for this data type is: " << +limit << std::endl;
        std::cout << std::endl;
}
